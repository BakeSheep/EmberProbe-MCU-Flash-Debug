...
const {
    ...
    compareVersions,
    checkCompatibility
} = require("../skills/_emberprobe/openocd-policy");
        ...
        new Promise((resolve) => {
            ...
            try {
                ...
                resolve({
                    ...
                    requested: target,
                    version: "",
                    error: error.code === "ENOENT" ? "找不到该可执行文件" : error.message,
                    errorKey: error.code === "ENOENT" ? "oc.errNotFound" : undefined
                });
                return;
            }
            let stdout = "";
            ...
            child.on("close", (code) => {
                ...
                if (version || /open on-chip debugger/i.test(out)) {
                        ...
                        requested: target,
                        version: "",
                        error: code ? `退出码 ${code}，且输出无法识别为 OpenOCD` : "所选文件不是有效的 OpenOCD",
                        errorKey: code ? "oc.errExit" : "oc.errNotOpenocd",
                        errorParams: code ? { code } : undefined
                    });
                }
            });
        });
...
function setCache(result) {
    _cachedResult = withCompatibility(result);
}
function resetCache() {
    _cachedResult = null;
}
...
async function persistOpenOcdPath(vscode, fsPath, previousPath) {
    ...
    const target = folders.length ? vscode.ConfigurationTarget.Workspace : vscode.ConfigurationTarget.Global;
...
async function pickOpenOcdPath(vscode, context, report, lang) {
    ...
    if (isCompatibleResult(result)) {
        ...
        reportStatus(report, {
            state: "ready",
            key: result.version ? "oc.readyVer" : "oc.ready",
    ...
    reportStatus(report, {
        state: "error",
        key: "oc.selectedUnusable",
        params: {
            error: result.errorKey
                ? i18n.t(lang, result.errorKey, result.errorParams)
                : result.error || i18n.t(lang, "oc.reselect")
        },
        result
    });
    return null;
...
async function installBundledAndConfigure(vscode, context, report, lang) {
    ...
    try {
        ...
        const result = await installer.installBundledOpenOcd(
            vscode,
            context,
            (p) => {
                if (p.message || p.key)
                    reportStatus(report, { state: "installing", key: p.key, params: p.params, message: p.message });
            },
            async (candidate) => {
                ...
                return {
                    ...
                    error:
                        stagedProbe.found && !stagedProbe.compatible
        ...
        if (!result.ok) {
            reportStatus(report, {
                state: "error",
                key: "oc.installFailed",
                params: { error: result.error || i18n.t(lang, "oc.unknownError") }
            });
            return null;
        }
        ...
        if (isCompatibleResult(verify)) {
            ...
            reportStatus(report, {
                state: "ready",
                key: verify.version ? "oc.installedReadyVer" : "oc.installedReady",
                params: { version: verify.version },
                result: verify
            });
            return result.path;
        }
        ...
        reportStatus(report, {
            state: "error",
            key: "oc.verifyFailed",
            params: { error: verify.error || i18n.t(lang, "oc.confirmManually") },
            result: verify
        });
        return null;
    } catch (error) {
...
async function resolveOpenOcdStatus(executable, context, probedResult, report) {
    ...
    const rawResult =
        probedResult && (probedResult.requested || probedResult.path) === target
            ? probedResult
