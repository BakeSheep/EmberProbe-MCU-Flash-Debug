...
(async () => {
    ...
    try {
        ...
        assert.strictEqual(await inspectFirmware(config, { ...options, platform: "darwin" }), null);
        assert.strictEqual((await inspectFirmware(config, { ...options, platform: "linux" })).installed, false);
