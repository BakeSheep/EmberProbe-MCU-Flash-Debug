...
(async () => {
    ...
    assert.equal(renders.length, 1, "unchanged focus refresh retains the existing document");
    provider.getModernWebviewContent = () => "changed";
    ...
    await provider.updateView();
    assert.equal(renders.length, 2, "completed unchanged refresh does not rebuild status widgets");
    provider.getModernWebviewContent = () => "closing";
