...
- J-Link requires explicit `swd` or `jtag`; `auto` returns `PROBE_TRANSPORT_REQUIRED`. `--probe-serial <decimal>` selects the physical J-Link and `--adapter-speed-khz <integer>` sets kHz (`0` keeps script defaults). Report and preserve these values from preflight through confirmation and execution. A changed identity invalidates confirmation.
